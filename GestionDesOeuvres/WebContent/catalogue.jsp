		...
		<c:forEach items="${listeOeuvres}" var="current">
			<tr>
				...
				<td><div class="btn-group">
						<c:choose>
							<c:when test="${current.etatOeuvrevente eq 'L'}">
								<a class="btn btn-mini btn-primary"
									href="Controller?action=reserveOeuvre&id=${current.idOeuvrevente}">
									<i class="icon-pencil"></i> Réserver
								</a>
								<a class="btn btn-mini btn-danger"
									href="Controller?action=modifOeuvre&id=${current.idOeuvrevente}">
									<i class="icon-trash"></i> Modifier
								</a>
							</c:when>
							<c:otherwise>
